...
func ExportJadwalCutiToExcel(c *gin.Context) {
	...
	c.Header("Content-Disposition", "attachment; filename=its_report_jadwalCuti.xlsx")
	c.Header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
	c.Header("Content-Length", strconv.Itoa(len(buffer.Bytes())))
	c.Writer.Write(buffer.Bytes())
}

func setMonthDataCuti(f *excelize.File, sheet, month string, rowOffset, colOffset int, events_rapat []models.JadwalCuti) {
	var (

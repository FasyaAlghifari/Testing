...
export const ReusableTable = ({
  ...
  const fetchUploadedFiles = async (id) => {
    try {
      const getFiles = UploadArsip.get
      const response = await fetch(`http://localhost:8080/${getFiles}/${id}`, {
        headers: {
  ...
  const uploadFileToServer = async (file) => {
    ...
    try {
      const uploadFiles = UploadArsip.upload;
  ...
  const AddSubmit = async (data) => {
    ...
    try {
      ...
      if (response.conflicts) {
        handleConflict(response.conflicts);
      } else {
        Swal.fire({
          icon: "success",
          title: "Berhasil!",
  ...
  const columns = [
    ...
    ...(InfoColumn
      ? [
        {
          name: "User Upload", // Menambahkan kolom Info
          selector: (row) => row.create_by, // Ganti 'info' dengan nama field yang sesuai dari data
          sortable: true,
        },
      ]
    ...
    ...(StatusColumn ? [{
      name: "Status", // Hanya satu kolom Status
      ...
      conditionalCellStyles: [
        ...
        {
          when: row => row.status === 'Reschedule',
          style: {
        ...
        {
          when: row => row.status === 'On Progress',
          style: {
		...
		return MainData.filter((item) => {
			...
			const matchesCategory =
				...
				selectedCategories.some((category) => {
					const noMemoMatches = item.no_memo && item.no_memo.includes(category);
					const noSuratMatches = item.no_surat && item.no_surat.includes(category);
					const codeProjectMatches = item.kode_project && item.kode_project.includes(category);
					return noMemoMatches || noSuratMatches || codeProjectMatches;
				});
			return matchesSearch && matchesCategory;
		});
	}, [MainData, globalFilterText, selectedCategories]);

	// FORM

	const onFilterChange = (categories) => {
            ...
            <>
              ...
              {filterSagIso && (
								<Dropdown
									...
									inline
									label={
										<div className="relative flex items-center bg-blue-400 w-full text-white p-[.5rem] rounded-lg">
											Filter
										</div>
									}>
									<Dropdown.Header>
										<div className="flex justify-center items-center gap-2">
											<Checkbox
												id="sag"
												value="ITS-SAG"
												checked={selectedCategories.includes("ITS-SAG")}
												onChange={() => handleCategoryChange("ITS-SAG")}
											/>
											<Label htmlFor="sag" className="text-base">
												ITS-SAG
											</Label>
										</div>
									</Dropdown.Header>
									<Dropdown.Header>
										<div className="flex justify-center items-center gap-2">
											<Checkbox
												id="iso"
												value="ITS-ISO"
												checked={selectedCategories.includes("ITS-ISO")}
												onChange={() => handleCategoryChange("ITS-ISO")}
											/>
											<Label htmlFor="sag" className="text-base">
												ITS-ISO
											</Label>
...
// Komponen modal untuk menampilkan konflik
const ConflictModal = ({ isOpen, onClose, conflicts }) => {
  return (
    <Modal show={isOpen} onClose={onClose}>
      <Modal.Header>Konflik Jadwal</Modal.Header>
      <Modal.Body>
        <ul>
          {conflicts.map((conflict, index) => (
            <li key={index}>
              {conflict.title} - {conflict.start} to {conflict.end}
            </li>
          ))}
        </ul>
      </Modal.Body>
